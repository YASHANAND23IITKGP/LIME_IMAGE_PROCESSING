...
if flag==1
    subplot(2,3,1);imshow(img_in);title('input image');
    subplot(2,3,2);colormap('hot');imagesc(Ti(:,:,1));colorbar;title('Ti'); %heat map
    subplot(2,3,3);colormap('hot');imagesc(Tout(:,:,1));colorbar;title('Tout');
    subplot(2,3,4);imshow(img_out);title('Output image');
    subplot(2,3,5);imshow(Iout);title('Output image after filtering');
end
